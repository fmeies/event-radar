...
async def register(
    ...
    if len(password) < MIN_PASSWORD_LENGTH:
        return templates.TemplateResponse(
            "register.html",
            {"request": request, "user": None, "error": "Password must be at least 8 characters long."},
        )

    if db.query(User).filter(User.email == email).first():
        return templates.TemplateResponse(
            "register.html",
            {"request": request, "user": None, "error": "An account with this email address already exists."},
        )

    token = generate_verification_token(email)
    new_user = User(email=email, password_hash=hash_password(password), verification_token=token)
    db.add(new_user)
    db.commit()
    ...
    try:
        ...
        return templates.TemplateResponse(
            "register.html",
            {"request": request, "user": None, "error": "Could not send confirmation email. Please try again later."},
        )
...
async def verify_email(request: Request, token: str, db: Session = Depends(get_db)):
    ...
    if not email:
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "user": None, "error": "This confirmation link is invalid or has expired."},
        )
...
async def login_page(request: Request, msg: str = "", user=Depends(get_current_user)):
    ...
    success = ""
    if msg == "registered":
        success = "Account created! Please check your inbox and confirm your email address."
    elif msg == "verified":
...
async def login(
    ...
    if not user or not verify_password(password, user.password_hash):
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "user": None, "error": "Invalid email address or password."},
        )

    if not user.is_verified:
        return templates.TemplateResponse(
            "login.html",
            {"request": request, "user": None, "error": "Please confirm your email address before signing in."},
        )
