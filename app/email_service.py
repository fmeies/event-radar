...
async def _send(msg: EmailMessage) -> None:
    kwargs: dict[str, Any] = {"hostname": settings.smtp_host, "port": settings.smtp_port}
    ...
    log.debug("Sending email to %s via %s:%d", msg["To"], settings.smtp_host, settings.smtp_port)
...
async def send_event_notification(to_email: str, events: list[dict], location: str) -> None:
    log.info("Sending event notification to %s (%d event(s) in %s)", to_email, len(events), location)
    lines: list[str] = []
    for e in events:
